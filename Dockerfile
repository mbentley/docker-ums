...
ENV UMSVER 3.6.4
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y dcraw mediainfo mencoder mplayer openjdk-7-jre vlc wget

RUN dpkg --add-architecture i386
RUN apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y libfreetype6:i386 libstdc++6:i386
